...
class Piotroski(Valuation):
  ...
  def valuate(self):
    ...
    values.append((
        1 if (data['OPERATING_PROFIT_AFTER_TAX'].dropna()[:1][0] * 100000000) >
        (data['NET_PROFIT_DURING_A_YEAR'].dropna()[:1][0] * stocks[0]) else 0))
    ...
    values.append(1
                  if gross_profit_margins[1] < gross_profit_margins[0] else 0)
